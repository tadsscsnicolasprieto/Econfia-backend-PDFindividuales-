...
NOMBRE_SITIO = "opensanctions_us_cuba"  # asegúrate de crear la Fuente con este nombre
URL_SEARCH = "https://www.opensanctions.org/search/?scope=us_cuba_sanctions&q={q}"
GOTO_TIMEOUT_MS = 180_000

# Selectores robustos (clases con hashes)
...
async def consultar_opensanctions_us_cuba(consulta_id: int, nombre: str, apellido: str):
    navegador = None
    ...
    success = False
    last_error = None
    score_final = 1  # por defecto (sin match exacto)
    ...
    try:
        async with async_playwright() as p:
            ...
            # 4) Navegar a la búsqueda con scope us_cuba_sanctions
            ...
            if await nores.count() > 0 and await nores.first.is_visible():
                ...
                # 6) Hay resultados: iterar títulos y buscar coincidencia exacta
