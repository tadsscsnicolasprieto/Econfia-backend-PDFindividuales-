...
NOMBRE_SITIO = "opensanctions_za_fic"  # asegúrate que exista en tu tabla Fuente
URL_SEARCH = "https://www.opensanctions.org/search/?scope=za_fic_sanctions&q={q}"
...
def _norm(s: str) -> str:
    """Normaliza strings para comparación exacta flexible (lower, sin tildes, espacios comprimidos)."""
    s = (s or "").strip().lower()
    s = "".join(c for c in unicodedata.normalize("NFD", s) if unicodedata.category(c) != "Mn")
    s = re.sub(r"\s+", " ", s)
    return s

async def consultar_opensanctions_za_fic(consulta_id: int, nombre: str, apellido: str):
    navegador = None
    ...
    try:
        async with async_playwright() as p:
            ...
            context = await navegador.new_context(
                ...
                locale="en-US",
                timezone_id="Africa/Johannesburg",  # opcional, pero es Sudáfrica
            )
            page = await context.new_page()

            # 3) Buscar con scope=za_fic_sanctions
            ...
            if await nores.count() > 0 and await nores.first.is_visible():
                ...
                # 5) Hay resultados: revisar coincidencia exacta por título
