# bots/opensanctions_ps_local_freezing.py
...
NOMBRE_SITIO = "opensanctions_ps_local_freezing"  # registra este nombre en Fuente
URL_SEARCH = "https://www.opensanctions.org/search/?scope=ps_local_freezing&q={q}"
GOTO_TIMEOUT_MS = 180_000

# Selectores robustos a cambios en hash de clases
SEL_ALERT_NORES = "div.alert-heading.h4"                   # "No matching entities were found."
...
async def consultar_opensanctions_ps_local_freezing(consulta_id: int, nombre: str, apellido: str):
    navegador = None
    ...
    try:
        async with async_playwright() as p:
            ...
            # 3) Buscar con scope=ps_local_freezing
            ...
            if await nores.count() > 0 and await nores.first.is_visible():
                ...
                # 5) Con resultados: verificar coincidencia exacta en títulos
                ...
                for i in range(n):
                    item = items.nth(i)
                    try:
                        title = (await item.locator(SEL_TITLE_A).first.inner_text(timeout=3000)).strip()
                    except Exception:
                        title = ""
        ...
        if success:
            ...
            await sync_to_async(Resultado.objects.create)(
                ...
                mensaje="No fue posible obtener resultados.", archivo=relative_png
            )
        ...
        try:
            await sync_to_async(Resultado.objects.create)(
                consulta_id=consulta_id, fuente=fuente_obj,
                score=1, estado="Sin Validar", mensaje=str(e), archivo=""
            )
        finally:
            try:
