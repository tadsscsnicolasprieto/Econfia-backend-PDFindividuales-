# bots/opensanctions_jp_meti_eul.py
...
NOMBRE_SITIO = "opensanctions_jp_meti_eul"  # asegúrate de tener esta Fuente en BD
URL_SEARCH = "https://www.opensanctions.org/search/?scope=jp_meti_eul&q={q}"
...
async def consultar_opensanctions_jp_meti_eul(consulta_id: int, nombre: str, apellido: str):
    navegador = None
    ...
    try:
        async with async_playwright() as p:
            ...
            # 3) URL de búsqueda con scope=jp_meti_eul
            ...
            if await nores.count() > 0 and await nores.first.is_visible():
                ...
                try:
                    ...
                    pass
                success = True  # válida, sin hallazgos (score=1)

            else:
                # 5) Hay resultados: iterar títulos y buscar match exacto
