# bots/opensanctions_us_occ_enfact.py
...
NOMBRE_SITIO = "opensanctions_us_occ_enfact"  # crea/asegura esta Fuente en tu DB
URL_SEARCH = "https://www.opensanctions.org/search/?scope=us_occ_enfact&q={q}"
GOTO_TIMEOUT_MS = 180_000

# Selectores robustos (clases con sufijos hash)
...
def _norm(s: str) -> str:
    """ Normaliza: lower + sin tildes + espacios comprimidos """
    s = (s or "").strip().lower()
    s = "".join(c for c in unicodedata.normalize("NFD", s) if unicodedata.category(c) != "Mn")
    s = re.sub(r"\s+", " ", s)
    return s

async def consultar_opensanctions_us_occ_enfact(consulta_id: int, nombre: str, apellido: str):
    navegador = None
    ...
    success = False
    last_error = None
    score_final = 1  # por defecto 1; exact match => 5
    ...
    try:
        async with async_playwright() as p:
            navegador = await p.chromium.launch(
                headless=True,  # lo pediste explícitamente
            ...
            # 3) Ir a la búsqueda con scope US OCC Enforcement Actions
            ...
            if await nores.count() > 0 and await nores.first.is_visible():
                ...
                try:
                    ...
                    pass
                success = True  # sin hallazgos, score=1

            else:
                # 5) Hay resultados: iterar títulos y comparar exacto
