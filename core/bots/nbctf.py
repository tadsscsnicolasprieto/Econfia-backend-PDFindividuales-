...
# En SharePoint los items suelen tener sufijos variables; cubrimos ambos casos:
...
async def consultar_nbctf(consulta_id: int, nombre: str, apellido: str):
    navegador = None
    ...
    try:
        async with async_playwright() as p:
            ...
            # 3) Construir URL de búsqueda (como tu ejemplo)
            base_u = "https://nbctf.mod.gov.il"
            search_url = URL_SEARCH_TPL.format(
                u=urllib.parse.quote(base_u, safe=""),
            ...
            if await nores.count() > 0 and await nores.first.is_visible():
                ...
                # 5) Hay resultados: iterar items
                # Preferimos el selector estricto; si no hay, usamos el loose.
                ...
                for i in range(n):
                    item = items.nth(i)
                    # Tomamos texto visible del contenedor; incluye título + snippet
                    try:
                    ...
                    if blob and _norm(blob).find(norm_query) != -1:
                        # Para "exacto", exigimos igualdad exacta del nombre con algún token de título.
                        # SharePoint suele contener el nombre literal en el bloque; validamos igualdad estricta
                        # sobre líneas separadas (robustez).
