# bots/opensanctions_ebrd_ineligible.py
...
NOMBRE_SITIO = "opensanctions_ebrd_ineligible"  # asegúrate de tener esta Fuente
URL_SEARCH = "https://www.opensanctions.org/search/?scope=ebrd_ineligible&q={q}"
GOTO_TIMEOUT_MS = 180_000

# Selectores robustos (clases con hash cambiante)
...
def _norm(s: str) -> str:
    """Normaliza: minúsculas, sin diacríticos y espacios comprimidos."""
    s = (s or "").strip().lower()
    s = "".join(c for c in unicodedata.normalize("NFD", s) if unicodedata.category(c) != "Mn")
    s = re.sub(r"\s+", " ", s)
    return s

async def consultar_opensanctions_ebrd_ineligible(consulta_id: int, nombre: str, apellido: str):
    navegador = None
    ...
    try:
        async with async_playwright() as p:
            ...
            # 3) Ir a búsqueda (scope=ebrd_ineligible)
            ...
            if await nores.count() > 0 and await nores.first.is_visible():
                try:
                    mensaje_final = (await nores.first.inner_text()).strip() or "No matching entities were found."
                except Exception:
                ...
                try:
                    ...
                    pass
                success = True  # score_final queda en 1

            else:
                # 5) Iterar resultados y buscar match exacto en el título
