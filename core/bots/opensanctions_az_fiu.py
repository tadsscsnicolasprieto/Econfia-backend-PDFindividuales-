...
NOMBRE_SITIO = "opensanctions_az_fiu"  # crea/asegura esta Fuente en tu BD
URL_SEARCH = "https://www.opensanctions.org/search/?scope=az_fiu_sanctions&q={q}"
GOTO_TIMEOUT_MS = 180_000

# Selectores robustos contra clases con hash
...
def _norm(s: str) -> str:
    """Normaliza para comparación exacta: minus, sin tildes, espacios comprimidos."""
    s = (s or "").strip().lower()
    s = "".join(c for c in unicodedata.normalize("NFD", s) if unicodedata.category(c) != "Mn")
    s = re.sub(r"\s+", " ", s)
    return s

async def consultar_opensanctions_az_fiu(consulta_id: int, nombre: str, apellido: str):
    navegador = None
    ...
    try:
        async with async_playwright() as p:
            ...
            # 3) Ir a la URL de búsqueda (scope=az_fiu_sanctions)
