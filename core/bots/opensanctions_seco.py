...
NOMBRE_SITIO = "opensanctions_seco"  # agrega este nombre en tu tabla Fuente
URL_SEARCH = "https://www.opensanctions.org/search/?scope=ch_seco_sanctions&q={q}"
...
def _norm(s: str) -> str:
    """Normaliza texto para comparación exacta: minus, sin tildes, espacios comprimidos."""
    s = (s or "").strip().lower()
    s = "".join(c for c in unicodedata.normalize("NFD", s) if unicodedata.category(c) != "Mn")
    s = re.sub(r"\s+", " ", s)
    return s

async def consultar_opensanctions_seco(consulta_id: int, nombre: str, apellido: str):
    navegador = None
    ...
    success = False
    last_error = None
    score_final = 1  # por defecto 1; si hay match exacto => 5
    ...
    try:
        async with async_playwright() as p:
            ...
            # 3) Ir a la URL de búsqueda (scope=ch_seco_sanctions)
