...
NOMBRE_SITIO = "ofsi_govuk"  # agrega este nombre en tu tabla Fuente
...
SEL_LIST_ITEM     = "li.gem-c-document-list__item"   # cada resultado
SEL_TITLE_LINK    = f"{SEL_LIST_ITEM} h3 a, {SEL_LIST_ITEM} a"  # link con el título
...
async def consultar_ofsi_govuk(consulta_id: int, nombre: str, apellido: str):
    navegador = None
    ...
    try:
        async with async_playwright() as p:
            navegador = await p.chromium.launch(
                headless=True,  # para ver el flujo si quieres
            ...
            if n == 0:
                # Sin UL/LI de resultados → no results
                mensaje_final = "No results found."
                success = True
                try:
        ...
        # 8) Guardar resultado
        estado_final = "Validada" if success else "Sin Validar"
        await sync_to_async(Resultado.objects.create)(
