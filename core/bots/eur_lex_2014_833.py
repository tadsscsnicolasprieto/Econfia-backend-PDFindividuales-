...
NOMBRE_SITIO = "eur_lex_2014_833"  # crea/usa esta Fuente en tu tabla
...
async def _clean_page_css(page):
    """Oculta header/footer/cookies para que el pantallazo quede centrado en los resultados."""
    css = """
...
async def consultar_eur_lex_2014_833(consulta_id: int, nombre: str, apellido: str):
    navegador = None
    ...
    try:
        async with async_playwright() as p:
            navegador = await p.chromium.launch(
                headless=True,  # cámbialo a False si quieres ver el navegador
            ...
            # 3) Construir URL de búsqueda (qid cambia siempre)
            q = urllib.parse.quote_plus(full_name)
            qid = str(int(time.time() * 1000))  # milisegundos actuales
            ...
            if await nores.count() > 0 and await nores.first.is_visible():
                ...
                # 5) Hay resultados: iterar cada div.SearchResult y buscar el nombre exacto
                results = page.locator(SEL_RESULT)
                # Asegura un pequeño wait por si carga por AJAX
                try:
