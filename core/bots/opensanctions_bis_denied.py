# bots/opensanctions_bis_denied.py
...
NOMBRE_SITIO = "opensanctions_bis_denied"  # Asegúrate de tenerla en tu tabla Fuente
URL_SEARCH = "https://www.opensanctions.org/search/?scope=us_bis_denied&q={q}"
GOTO_TIMEOUT_MS = 180_000

# Selectores robustos (clases ofuscadas: usamos 'contains')
...
async def consultar_opensanctions_bis_denied(consulta_id: int, nombre: str, apellido: str):
    navegador = None
    ...
    score_final = 1           # 1 por defecto; 5 si hay match exacto
    success = False
    last_error = None
    ...
    try:
        async with async_playwright() as p:
            navegador = await p.chromium.launch(
                headless=True,  # <-- pedido explícito
            ...
            # 4) Ir a la búsqueda con scope=us_bis_denied
            ...
            # 5) Detectar "No matching entities were found."
            ...
            if await nores.count() > 0 and await nores.first.is_visible():
                ...
                # 6) Iterar resultados y buscar coincidencia exacta con el título (<a>)
