...
NOMBRE_SITIO = "opensanctions_us_ddtc_debarred"  # asegúrate de tener esta Fuente en BD
URL_SEARCH = "https://www.opensanctions.org/search/?scope=us_ddtc_debarred&q={q}"
...
def _norm(s: str) -> str:
    """
    Normaliza: minúsculas, sin tildes, espacios comprimidos, trim.
    """
    s = (s or "").strip().lower()
...
async def consultar_opensanctions_us_ddtc_debarred(consulta_id: int, nombre: str, apellido: str):
    navegador = None
    ...
    try:
        async with async_playwright() as p:
            ...
            # 3) Ir a la URL de búsqueda (scope us_ddtc_debarred)
            ...
            if await nores.count() > 0 and await nores.first.is_visible():
                ...
                # 5) Hay lista de resultados -> revisar títulos (anchor)
