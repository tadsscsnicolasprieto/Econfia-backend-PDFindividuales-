...
async def consultar_state_designation_cartels_pdf(consulta_id: int, nombre: str, cedula: str):
    # Construir URL con el nombre (similar a OFAC, aquí usamos el nombre completo)
    query = nombre.replace(" ", "+")
    url = f"https://findit.state.gov/search?query={query}&affiliate=dos_stategov"
    ...
    # Buscar coincidencias de la cédula y del nombre en el texto de la página
    texto_pagina = soup.get_text()
    coincidencias_cedula = texto_pagina.count(cedula) if cedula else 0
    coincidencias_nombre = texto_pagina.lower().count(nombre.lower())

    total_coincidencias = coincidencias_cedula + coincidencias_nombre

    if total_coincidencias > 2:
        estado = "validado"
        mensaje = f"Se encontraron {total_coincidencias} coincidencias para {cedula} - {nombre}"
        score = 5
    else:
        estado = "validado"
    ...
    draw.text((20, 40), f"Página: {NOMBRE_SITIO} - findit.state.gov", fill="black", font=font)
