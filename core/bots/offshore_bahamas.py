...
URL = "https://offshoreleaks.icij.org/investigations/bahamas-leaks"
NOMBRE_SITIO = "offshore_bahamas"

def _norm_name(s: str) -> str:
    """Normaliza nombre para comparación 'exacta visual':
       - quita diacríticos, may/min insensible, colapsa espacios,
       - normaliza coma con espacio (', '), y hace strip()."""
    if not s:
        return ""
...
async def consultar_offshore_bahamas(consulta_id: int, nombre: str, cedula):
    """
    Abre Bahamas Leaks (ICIJ), busca 'nombre' y toma hasta 3 capturas.
    - Coincidencia exacta en columna 'Officer' => score=5
    - Sin coincidencia exacta => score=1
    Reintenta hasta 3 veces y siempre intenta guardar pantallazo en error.
    """
    # Carpeta/archivos
    ...
    for intento in range(1, 4):
        ...
        try:
            ...
            async with async_playwright() as p:
                ...
                # 4) Páginas: captura y verificación hasta 3
                for i in range(1, 4):
                    # ¿No results? (best effort)
                    ...
                    # Leer filas de la tabla principal
                    ...
                    if match_exacto or no_results:
                        # ya tenemos el veredicto; no hace falta seguir paginando
                        break
            ...
            # Guardar resultado exitoso (con mensaje)
            await sync_to_async(Resultado.objects.create)(
                consulta_id=consulta_id,
                fuente=fuente_obj,
                score=score,
