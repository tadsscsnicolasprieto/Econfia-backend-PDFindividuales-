...
NOMBRE_SITIO = "opensanctions_eu_fsf"  # crea la Fuente con este nombre
URL_SEARCH = "https://www.opensanctions.org/search/?scope=eu_fsf&q={q}"
GOTO_TIMEOUT_MS = 180_000

# Selectores (tolerantes a cambios de hash en clases)
...
def _norm(s: str) -> str:
    """Normaliza para comparación exacta: minúsculas, sin diacríticos, espacios comprimidos."""
    s = (s or "").strip().lower()
    s = "".join(c for c in unicodedata.normalize("NFD", s) if unicodedata.category(c) != "Mn")
    s = re.sub(r"\s+", " ", s)
    return s

async def consultar_opensanctions_eu_fsf(consulta_id: int, nombre: str, apellido: str):
    navegador = None
    ...
    success = False
    last_error = None
    score_final = 1  # por defecto (solo sube a 5 si hay match exacto)
    ...
    try:
        async with async_playwright() as p:
            ...
            # 3) Ir a la URL de búsqueda (scope=eu_fsf)
