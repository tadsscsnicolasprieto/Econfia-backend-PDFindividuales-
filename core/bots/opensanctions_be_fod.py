...
NOMBRE_SITIO = "opensanctions_be_fod"  # asegúrate de crear esta Fuente en BD
URL_SEARCH = "https://www.opensanctions.org/search/?scope=be_fod_sanctions&q={q}"
GOTO_TIMEOUT_MS = 180_000

# Selectores robustos (clases con sufijos hash, usamos contains)
...
async def consultar_opensanctions_be_fod(consulta_id: int, nombre: str, apellido: str):
    navegador = None
    ...
    success = False
    last_error = None
    score_final = 1  # por defecto 1 (solo sube a 5 con match exacto)
    ...
    try:
        async with async_playwright() as p:
            ...
            # 3) Buscar en scope be_fod_sanctions
            ...
            if await nores.count() > 0 and await nores.first.is_visible():
                ...
                # 5) Iterar resultados y buscar coincidencia exacta en el título
