# bots/cpiq_validacion_tarjeta.py
import os
import re
import asyncio
...
NOMBRE_SITIO = "cpiq_validacion_tarjeta"

# Selectores
SEL_CODIGO = '#id_number'  # <input type="text" name="id_number" id="id_number" ...>
SEL_RESULT_HINTS = [
    "text=Resultado de la validación",
    "text=se encuentra registrada",            # si la tarjeta es válida
    "text=no se encuentra registrada",         # si no existe
...
async def consultar_cpiq_validacion_tarjeta(
    consulta_id: int,
    numero: str  # aquí colocamos la cédula como "código de validación", tal como pediste
):
    """
    CPIQ – Validación de la tarjeta profesional digital:
      - Abre https://www.cpiq.gov.co/validacion_tarjeta_profesional.php
      - Ingresa el código (usaremos la cédula provista) y presiona Enter
      - Espera el render de resultados
      - Reescribe el código para que aparezca en el screenshot
    ...
    try:
        ...
        async with async_playwright() as p:
            ...
            # Navegar
            await page.goto("https://www.cpiq.gov.co/validacion_tarjeta_profesional.php",
            ...
            # Reescribir el código para que sea visible en el pantallazo
            try:
                await inp.fill("")
