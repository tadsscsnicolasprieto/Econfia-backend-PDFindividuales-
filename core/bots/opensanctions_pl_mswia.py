# bots/opensanctions_pl_mswia.py
...
NOMBRE_SITIO = "opensanctions_pl_mswia"  # asegúrate que exista en tu tabla Fuente
URL_SEARCH = "https://www.opensanctions.org/search/?scope=pl_mswia_sanctions&q={q}"
GOTO_TIMEOUT_MS = 180_000

# Selectores robustos (clases ofuscadas)
...
def _norm(s: str) -> str:
    """ normaliza: lower, sin tildes, trim, y espacios comprimidos """
    s = (s or "").strip().lower()
    s = "".join(c for c in unicodedata.normalize("NFD", s) if unicodedata.category(c) != "Mn")
    s = re.sub(r"\s+", " ", s)
    return s

async def consultar_opensanctions_pl_mswia(consulta_id: int, nombre: str, apellido: str):
    navegador = None
    ...
    try:
        async with async_playwright() as p:
            ...
            # 3) Ir a la URL de búsqueda con scope=pl_mswia_sanctions
            ...
            if await nores.count() > 0 and await nores.first.is_visible():
                ...
                # 5) Hay resultados: evaluar coincidencia exacta en títulos
